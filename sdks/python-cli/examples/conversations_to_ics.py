...
"""
Convert Omi conversation-list JSON export to iCalendar (.ics).

Usage:
    python conversations_to_ics.py INPUT.json OUTPUT.ics
"""
...
def convert(source, destination):
    ...
    if isinstance(data, dict):
        items = data.get("conversations") or data.get("items") or data.get("data") or [data]
...
if __name__ == "__main__":
    ...
    print(f"{written} event(s) written, {skipped} item(s) skipped (no started_at)")
