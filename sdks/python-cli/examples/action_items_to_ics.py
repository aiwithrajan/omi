...
"""
Convert Omi action-item list JSON export to iCalendar (.ics).

Usage:
    python action_items_to_ics.py INPUT.json OUTPUT.ics
"""
...
def convert(source, destination):
    ...
    if isinstance(data, dict):
        items = data.get("action_items") or data.get("items") or data.get("data") or [data]
    ...
    for item in items:
        ...
        description = ics_text(item.get("description") or item.get("title")) or "(no description)"
